...
export default function OptionsButton({text, onClickFunction, optionsValue, tooltip, itemsPerRow}: props){
    ...
    return(
        <div className="relative z-50 flex flex-col items-center group/options">
            {pressed && portalRoot && createPortal(
                <div
                    ref={menuRef}
                    style={{ position: "fixed", top: menuStyle.top, left: menuStyle.left }}
                    className={`z-[5000] w-fit h-fit p-2 bg-card-bg border-2 border-card-border rounded-xl shadow-card backdrop-blur-sm duration-200 ${
                        menuReady ? "opacity-100 scale-100 pointer-events-auto" : "opacity-0 scale-95 pointer-events-none"
                    }`}
                >
